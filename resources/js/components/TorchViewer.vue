...
function getAppLogoHtml(appName?: string, defaultIcon?: string): string {
  ...
  if (name.includes('twitter') || name.includes(' x ')) {
    return `<svg class="app-logo-svg" viewBox="0 0 24 24" width="15" height="15" fill="#000000" style="display:inline-block; vertical-align:-2px; margin-right:4px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`
  }
  if (name.includes('shopee')) {
    return `<svg class="app-logo-svg" viewBox="0 0 24 24" width="15" height="15" fill="#EE4D2D" style="display:inline-block; vertical-align:-2px; margin-right:4px;"><path d="M8.28 12.87c.07.65.65 1.15 1.48 1.15 1.15 0 1.55-.54 1.55-1.15 0-1.84-4.57-1.15-4.57-4.14 0-1.9 1.48-3.35 3.78-3.35 1.95 0 3.32.97 3.53 2.63l-1.94.3c-.07-.76-.68-1.22-1.58-1.22-1 0-1.48.54-1.48 1.08 0 1.77 4.58 1.08 4.58 4.11 0 2-1.55 3.38-3.9 3.38-2.52 0-3.67-1.26-3.85-2.95l1.98-.36.42.52zm8.44-8.82c3.08 0 5.6 2.5 5.6 5.6v7.35c0 3.08-2.5 5.6-5.6 5.6H8.44c-3.08 0-5.6-2.5-5.6-5.6v-7.35c0-3.08 2.5-5.6 5.6-5.6h8.44zm.1 1.7H8.34C6.1 5.75 4.3 7.55 4.3 9.77v7.1c0 2.24 1.8 4.05 4.05 4.05h8.54c2.24 0 4.05-1.8 4.05-4.05v-7.1c0-2.22-1.8-4.02-4.02-4.02z"/></svg>`
  }
  if (name.includes('tencent') || name.includes('pubg')) {
    return `<svg class="app-logo-svg" viewBox="0 0 24 24" width="15" height="15" fill="#2059c2" style="display:inline-block; vertical-align:-2px; margin-right:4px;"><path d="M11.96 0C5.352 0 0 5.376 0 12c0 6.623 5.353 12 11.96 12 6.608 0 12.04-5.377 12.04-12C24 5.376 18.568 0 11.96 0zm0 3.125c4.896 0 8.874 3.99 8.874 8.875 0 4.885-3.978 8.875-8.874 8.875-4.897 0-8.875-3.99-8.875-8.875 0-4.885 3.978-8.875 8.875-8.875z"/></svg>`
  }
...
.btn.blue { color: var(--info); border-color: var(--info-dim); }
.btn.green { color: var(--success); border-color: var(--success-dim); }
.btn.amber { color: var(--warning); border-color: var(--warning-dim); }
.btn.red { color: var(--danger); border-color: var(--danger-dim); }
.btn.danger-solid {
...
.metric-row .v { font-family: var(--font-mono); font-weight: 700; color: var(--text-1); }
.metric-row .v.green { color: var(--success); }
